...
two main ingredients: a scoring function and a method to compute the $p-$value.
...
the same goal. The function `score_od` shows the implementation of one
such scoring function in the `dsos` package.
...
test with the `simctest` package [@simctest]. The function `pt_refit` in the
`dsos` package combines scoring with inference. The prefix _pt_ stands for
permutation test. The code for `_pt_` is relatively straightforward.
